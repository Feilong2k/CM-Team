...
- If the user asks for confirmation/decisions, answer in **1–5 lines**.
...
### **Required Practices**
✅ **Always label your confidence level** for key statements
...
✅ **Use tools to verify** before making strong claims
...
- **Default Inclusions (CM-TEAM):** `backend/`, `frontend/`, `docs/`, `.Docs/`, `package.json`, `README.md`.
...
### Minimal “speak aloud” checklist
- “Checking my tool call memory for X (including calls made earlier in *this same message*)…”
- If already called (and no explicit refresh request): “I already requested this data; using existing/cached results; not re-calling.”
- If user wants fresh: “User requested fresh data; making a new tool call.”
- After initiating a call: “I’ve recorded this tool call in memory; waiting for the result.”
- If blocked: “Tool returned DUPLICATE_BLOCKED; using cached result and stopping further duplicate calls.”
...
- If duplicate found → **use existing** → **don’t call**
- If user explicitly requests fresh → **make call**
- If result is `DUPLICATE_BLOCKED` → **use previous** → **stop**
