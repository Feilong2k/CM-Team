...
/**
 * Create Feature 3 and its tasks in the database using direct SQL
 * with correct column names (cap, red instead of pvp_analysis, fap_analysis)
 */
...
async function createFeature3() {
  ...
  const featureData = {
    ...
    pcc: {},
    cap: {},  // CAP analysis (not pvp_analysis)
    red: {},  // RED analysis (not fap_analysis)
  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO features (
        project_id, external_id, title, status, basic_info, 
        pcc, cap, red, created_at, updated_at
    ...
    console.log(`Created feature: ${result.rows[0].external_id} (ID: ${result.rows[0].id})`);
...
async function createTasksForFeature3(featureId) {
  ...
  for (const task of tasks) {
    try {
      ...
      // Insert task with correct column names
      const result = await db.query(
        `INSERT INTO tasks (
          feature_id, external_id, title, status, basic_info, 
          pcc, cap, created_at, updated_at
      ...
      console.log(`Created task: ${result.rows[0].external_id} (ID: ${result.rows[0].id})`);
...
async function main() {
  try {
    ...
    console.log('   node backend/scripts/create_subtasks_from_json.js backend/template/F3-two_stage_protocol_service_foundation.json');
